...
import { cn, formatDayName } from '@/lib/utils';
...
interface EventCardProps {
  event: EventSummary;
  featured?: boolean;
  /** % real vendido del evento (si no se pasa, se calcula con available_tickets) */
  soldCount?: number;
  capacity?: number;
}
...
export default function EventCard({
  ...
  // Para el badge de urgencia necesitamos sold/capacity.
  // Si no vienen por props, usamos available como proxy.
  const capacityValue = capacity ?? 100;
  const soldValue =
    soldCount !== undefined ? soldCount : Math.max(0, capacityValue - event.available_tickets);
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-gradient-to-br from-red-950/40 via-black to-red-950/20">
        {event.cover_url ? (
          <Image
            src={event.cover_url}
            alt={event.name}
